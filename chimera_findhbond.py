...
if __name__ == "__main__":
    ...
    Remember to set "domain" to the name of the domain to be processed - un-silence the next line and modify accordingly.
    '''
    # domain = sys.argv[1]
    domain = 'A3A'
    tarfile = 'pdb/' + domain + '_sample_pdbs.tar.gz'
    for pdb in ['5keg', '5sww', '5td5', '6bux']:
        subprocess.call('mkdir ' + domain + '_graft' + pdb, shell=True )
        subprocess.call('tar -xzf ' + tarfile + ' -C ' + domain + '_graft' + pdb + ' --wildcards *graft' + pdb + '*.pdb', \
            shell=True)
        ...
        subprocess.call('mv ' + domain + '_graft' + pdb + '_hbonds.tar.gz output/', shell=True)
        subprocess.call('rm -rf '+ domain + '_graft' + pdb + '/', shell=True)
        subprocess.call('rm *.hbonds', shell=True)
